...
export default function Home() {
  ...
  // Load and filter Inbox tasks when search modal opens
  const [searchTasks, setSearchTasks] = useState<Task[]>([]);
  useEffect(() => {
    const fetchSearchTasks = async () => {
      if (isSearchOpen) {
        // Fetch Inbox tasks (tasks without due date or completed = false)
        const inboxTasks = await queryClient.fetchQuery({
          queryKey: ["tasks", "Inbox"],
          queryFn: () => getTasks(undefined, undefined, undefined, false), // No due date, not completed
        });
            ...
            <CommandList>
              ...
              <CommandGroup heading="Tasks">
                {searchTasks.map((task) => (
                  <CommandItem
                    key={task.id} // Unique key since we're only using Inbox tasks
                    onSelect={() => {
                      handleSearchSelect(task.title);
                      setActiveItem("Inbox");
                    }}
