...
export default function Home() {
  ...
  /* --- lifecycle --- */
  useEffect(() => {
    setIsVisible(isTaskModalOpen || isProjectModalOpen);
  }, [isTaskModalOpen, isProjectModalOpen]);
  ...
  /* --- handlers (kept for future route pages) --- */
  const handleProjectSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newProjectName.trim()) {
    ...
    setProjects((p) => [...p, { id, name: newProjectName.trim() }]);
    setNewProjectName("");
    setIsProjectModalOpen(false);
  ...
  const handleCommandSelect = (value: string) => {
    handleSearchSelect(value);
    if (value === "add-task") setIsTaskModalOpen(true);
    else if (value === "add-project") setIsProjectModalOpen(true);
    setIsSearchOpen(false);
  };
  ...
  const fetchSearchTasks = async () => {
    ...
    const filtered = (inboxTasks || []).filter(
      (t) =>
  ...
  useEffect(() => {
    if (isSearchOpen) fetchSearchTasks();
  }, [isSearchOpen, searchTerm, queryClient]);

  /* --- render fallback / welcome --- */
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold">Welcome</h2>
      <p>Select an item to view its content.</p>
